...
async def test_rule_manager_client_payload(respx_mock):
    ...
    context = {
        ...
        "scenario_id": 1,
        "world_snapshot": {"entities": ["player", "orc"]},
    }
    ...
    # RuleRequestEntity uses Field(alias="entity_id")
    assert payload["entities"][0]["entity_id"] == "player"
...
async def test_scenario_manager_client_payload(respx_mock):
    ...
    # Mock Scenario Service /api/v1/check/session
    ...
    result = await client.get_proposal("User input", rule_outcome)
